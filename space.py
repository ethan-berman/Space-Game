...
can = tk.Canvas(win)
can.pack()
img = tk.PhotoImage("/background.png")
    ...
    if choice == '1':
        if(vessel.weapons > opponent.shields and opponent.attack > vessel.shields):
            ...
            if(vessel.weapons > opponent.attack):
                ...
                print("Your opponent destroyed your ship.  Nice try Captain " + vessel.name)
                running = False
                vessel.health = 0
                return("death")
        print('fight')
...
def choose_action(vessel, act):
    if act < 5:
        ...
        pass
        #case for a strong enemy
    elif act > 18:
        pass
        #case for a rare discovery of supplies

def shop(vessel):
    print("Welcome, " + vessel.captain + " To the shop!  You can upgrade your ship here.")
...
if running is True:
    while True:
        command = input("Would you like to: (1) Check your Ship's Vitals or (2) Advance the campaign or (3) Visit the shop?")
        if command == "1":
